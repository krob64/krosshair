...
unsigned char vert_spv[] = {
    ...
    0x38, 0x00, 0x01, 0x00};
unsigned int vert_spv_len = 1060;
 ...
 * */
int vk_map_set(vk_object_map_t* map, uint64_t obj, void* data) {
...
void vk_map_print(vk_object_map_t* map) {
    ...
    for (size_t i = 0; i < MAX_VK_OBJECTS; i++) {
        ...
        printf("item %lu has obj: \t\t %lu, data \t\t %p\n", i,
               map->data[i].obj, map->data[i].data);
    }
    printf(
        "######################################################################"
        "#####################\n");
...
int vk_obj_compare(const void* a, const void* b, void* udata) {
    const vk_object_t* oa = a;
    const vk_object_t* ob = b;
    return oa->obj == ob->obj;
}

uint64_t vk_obj_hash(const void* item, uint64_t seed0, uint64_t seed1) {
    const vk_object_t* obj = item;
    return hashmap_sip(&obj->obj, sizeof(obj->obj), seed0, seed1);
}

struct hashmap* vk_object_to_data = NULL;
...
typedef struct swapchain_data {
    ...
    VkImage images[16];  // TODO: check if this shit actually works or if a
                         // dynamic array is needed
    ...
    // std::list<krosshair_draw_t*> draws ?
    krosshair_draw_t* draw;

    uint64_t n_frames;  // TODO: check if this is actually needed;
} swapchain_data_t;
...
vertex_t vertices[] = {{{-texture_correct_width, -height}, {1.0f, 0.0f, 0.0f}},
                       {{texture_correct_width, -height}, {0.0f, 0.0f, 0.0f}},
                       {{texture_correct_width, height}, {0.0f, 1.0f, 0.0f}},
                       {{-texture_correct_width, height}, {1.0f, 1.0f, 1.0f}}};
...
static void print_hashmap(struct hashmap* map) {
    ...
    while (hashmap_iter(map, &iter, &item)) {
        const vk_object_t* obj = item;
        if (!obj) {
            printf("[hashmap_iter] item %lu is %p\n", iter, obj);
            continue;
        }
        printf("[hashmap_iter] item %lu is %lu with data %p\n", iter, obj->obj,
               obj->data);
    }
    printf(
        "######################################################################"
...
static void* find_object_data(uint64_t obj) {
    ...
    // printf("[hashmap_get] attempting to find object %lu\n", obj);
    // const vk_object_t* temp_obj =
    //     hashmap_get(vk_object_to_data, &(vk_object_t){.obj = obj});
    // printf("[hashmap_get] temp_obj: %p\n", temp_obj);
    // if (!temp_obj) {
    ...
    // pthread_mutex_unlock(&global_lock);
    pthread_mutex_lock(&global_lock);
    // printf("[find_object_data] attempting to find obj: %lu\n", obj);
    ...
    // printf("[find_object_data] found obj: %lu, data: %p\n\n", temp_obj.obj,
    // temp_obj.data);
    pthread_mutex_unlock(&global_lock);
    return temp_obj.data;
}
...
static void map_object(uint64_t obj, void* data) {
    ...
    // ##########################################\n");
    // pthread_mutex_lock(&global_lock);
    // printf("mapping obj \t\t %lu with data \t\t %p\n", obj, data);
    // vk_object_t* item = malloc(sizeof(*item));
    // item->obj = obj;
    // item->data = data;
    // hashmap_set(vk_object_to_data, item);
    // printf("##################################################################################################\n\n");
    // print_hashmap(vk_object_to_data);
    // pthread_mutex_unlock(&global_lock);
    printf(
        "########################################## [map_object] "
        "##########################################\n");
    ...
    if (vk_map_set(&vk_obj_map, obj, data) == -1) {
        pthread_mutex_unlock(&global_lock);
        return;
    }
    printf(
...
static void unmap_object(uint64_t obj) {
    printf(
        ...
        "##########################################\n");
    pthread_mutex_lock(&global_lock);
    // hashmap_delete(vk_object_to_data, &(vk_object_t){.obj = obj});
...
static device_data_t* new_device_data(VkDevice device,
                                      instance_data_t* instance) {
    device_data_t* data = malloc(sizeof(device_data_t));
    data->instance = instance;
    data->device = device;
    printf("[*] mapping data->device obj: %lu %p\n", HKEY(data->device), data);
...
static void create_or_resize_buffer(device_data_t* device_data,
    ...
    alloc_info.memoryTypeIndex = vk_memory_type(
        device_data, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, req.memoryTypeBits);
    VK_CHECK(device_data->vtable.AllocateMemory(device_data->device,
                                                &alloc_info, NULL, buffer_mem));

    VK_CHECK(device_data->vtable.BindBufferMemory(device_data->device, *buffer,
                                                  *buffer_mem, 0));
...
static void shutdown_krosshair_image(swapchain_data_t* data) {
    ...
    device_data->vtable.DestroyImage(device_data->device, data->crosshair_image,
                                     NULL);
...
static void create_image(swapchain_data_t* data,
                         VkDescriptorSet descriptor_set,
                         uint32_t width,
                         uint32_t height,
                         VkFormat format,
    ...
    device_data->vtable.GetImageMemoryRequirements(device_data->device, *image,
                                                   &kh_image_req);
    ...
    VK_CHECK(device_data->vtable.BindImageMemory(device_data->device, *image,
                                                 *image_mem, 0));
...
static VkDescriptorSet create_image_with_desc(swapchain_data_t* data,
    ...
    create_image(data, descriptor_set, width, height, format, image, image_mem,
                 image_view);
...
static void upload_image_data(device_data_t* device_data,
    ...
    VK_CHECK(device_data->vtable.MapMemory(device_data->device,
                                           *upload_buffer_mem, 0, upload_size,
                                           0, (void**)(&map)));
    ...
    VK_CHECK(device_data->vtable.FlushMappedMemoryRanges(device_data->device, 1,
                                                         &range));
    ...
    device_data->vtable.CmdPipelineBarrier(
        cmd_buffer, VK_PIPELINE_STAGE_HOST_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
    ...
    device_data->vtable.CmdCopyBufferToImage(
        cmd_buffer, *upload_buffer, image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
        1, &region);
...
static void ensure_swapchain_crosshair(swapchain_data_t* data,
    ...
    stbi_uc* pixels =
        stbi_load("/home/krob/crosshairs/ZeeqPlus1.png", &tex_width,
                  &tex_height, &tex_channels, STBI_rgb_alpha);
    image_size = tex_width * tex_height * 4;

    if (!pixels) {
        printf("[KROSSHAIR_ERROR] failed to load crosshair image.\n");
        return;
    }
    ...
    upload_image_data(device_data, cmd_buffer, pixels, image_size, tex_width,
                      tex_height, &data->crosshair_upload_buffer,
                      &data->crosshair_upload_buffer_mem,
                      data->crosshair_image);
...
static void create_draw(swapchain_data_t* data) {
    ...
    if (draw) {
        if (device_data->vtable.GetFenceStatus(device_data->device,
                                               draw->fence) == VK_SUCCESS) {
            VK_CHECK(device_data->vtable.ResetFences(device_data->device, 1,
                                                     &draw->fence));
            return;
        }
    }
    ...
    VK_CHECK(device_data->vtable.CreateSemaphore(
        device_data->device, &sem_info, NULL, &draw->crossengine_semaphore));
...
static krosshair_draw_t* render_swapchain_display(
    ...
    device_data->vtable.CmdBeginRenderPass(draw->cmd_buffer, &render_pass_info,
                                           VK_SUBPASS_CONTENTS_INLINE);

    // create vertex & index buffers
    // since our contents aren't changing at runtime, might as well do creation
    // and upload only once somewhere else? upload vertex & index data bind
    ...
    void* idx_dst = NULL;
    VK_CHECK(device_data->vtable.MapMemory(
        device_data->device, draw->index_buffer_mem, 0, draw->index_buffer_size,
        0, &idx_dst));
    ...
    device_data->vtable.CmdBindIndexBuffer(draw->cmd_buffer, draw->index_buffer,
    ...
    device_data->vtable.CmdDrawIndexed(draw->cmd_buffer, sizeof(indices), 1, 0,
                                       0, 0);

    // device_data->vtable.CmdDrawIndexed(draw->cmd_buffer, ); TODO
     ...
     * vkQueuePresent, insert our own cross-engine synchronization semaphore.
     * */
...
static void setup_swapchain_data_pipeline(swapchain_data_t* data) {
    ...
    VK_CHECK(device_data->vtable.CreateSampler(
        device_data->device, &sampler_info, NULL, &data->crosshair_sampler));
    ...
    VK_CHECK(device_data->vtable.CreateDescriptorPool(
        device_data->device, &desc_pool_info, NULL, &data->descriptor_pool));
    ...
    VK_CHECK(device_data->vtable.CreateDescriptorSetLayout(
        device_data->device, &set_layout_info, NULL, &data->descriptor_layout));
    ...
    attribute_desc[1].offset = offsetof(vertex_t, color);
    ...
    device_data->vtable.DestroyShaderModule(device_data->device, vert_module,
                                            NULL);
    device_data->vtable.DestroyShaderModule(device_data->device, frag_module,
                                            NULL);
...
static void setup_swapchain_data(swapchain_data_t* data,
    ...
    for (size_t i = 0; i < n_images; i++) {
        ...
        printf("[vkCreateImageView] n_images = %d\n", n_images);
        printf("[vkCreateImageView] %p\n", view_info.image);
        // TODO: destroy upon swapchain recreation
        VK_CHECK(device_data->vtable.CreateImageView(
            device_data->device, &view_info, NULL, &data->image_views[i]));
        printf("data->image_views[%lu]: %p\n", i, data->image_views[i]);
    }
    printf("###################################################################\n");
    ...
    for (size_t i = 0; i < n_images; i++) {
        ...
        VK_CHECK(device_data->vtable.CreateFramebuffer(
            device_data->device, &fb_info, NULL, &data->framebuffers[i]));
        printf("data->framebuffers[%lu]: %p\n", i, data->framebuffers[i]);
    }
    printf(
        "######################################################################"
        "##########\n");
    ...
    for (uint32_t i = 0; i < physical_device_count; i++) {
        if (map) {
            printf("[*] mapping physical_devices[%d] obj: %lu %p\n", i,
                   HKEY(physical_devices[i]), instance_data);
...
static swapchain_data_t* new_swapchain_data(VkSwapchainKHR swapchain,
                                            device_data_t* device_data) {
    // instance_data_t* instance_data = device_data->instance;
    swapchain_data_t* data = malloc(sizeof(*data));
...
static VkResult overlay_CreateInstance(const VkInstanceCreateInfo* pCreateInfo,
    ...
    vk_load_instance_commands(instance_data->instance, fpGetInstanceProcAddr,
...
static queue_data_t* new_queue_data(VkQueue queue,
                                    ...
                                    uint32_t family_index,
                                    device_data_t* device_data) {
    queue_data_t* data = malloc(sizeof(*data));
    memset(data, 0, sizeof(*data));
    data->device = device_data;
    data->queue = queue;
    data->flags = family_props->queueFlags;
    data->family_index = family_index;
    printf("[*] mapping data->queue obj: %lu %p\n", HKEY(data->queue), data);
    ...
    for (uint32_t i = 0; i < pCreateInfo->queueCreateInfoCount; i++) {
        for (uint32_t j = 0; j < pCreateInfo->pQueueCreateInfos[i].queueCount;
            ...
            data->vtable.GetDeviceQueue(
                data->device,
                pCreateInfo->pQueueCreateInfos[i].queueFamilyIndex, j, &queue);
...
static krosshair_draw_t* before_present(swapchain_data_t* swapchain_data,
    ...
    draw =
        render_swapchain_display(swapchain_data, present_queue, wait_semaphores,
...
static VkResult overlay_CreateDevice(VkPhysicalDevice physical_device,
                                     ...
                                     const VkAllocationCallbacks* pAllocator,
                                     VkDevice* pDevice) {
    instance_data_t* instance_data = FIND_OBJ(instance_data_t, physical_device);
    ...
    const char** enabled_extensions = malloc(
        sizeof(*enabled_extensions) * pCreateInfo->enabledExtensionCount);
    ...
    for (size_t i = 0; i < pCreateInfo->enabledExtensionCount; i++) {
        for (size_t j = 0; j < pCreateInfo->enabledExtensionCount; j++) {
            printf("available_extensions[%lu]: %s\n", i,
                   available_extensions[i].extensionName);
            printf("enabled_extensions[%lu]: %s\n", i, enabled_extensions[j]);
    ...
    if (found_extensions != pCreateInfo->enabledExtensionCount) {
        printf("[KROSSHAIR_ERROR] extensions don't match.\n");
    }
...
//     debug_file = fopen("/home/krob/krosshair.log", "w");
//     if (debug_file) {
//         fprintf(debug_file, "KROSSSHAIR loaded.\n");
//     }
//
//     fprintf(stderr, "[KROSSHAIR] CONSTRUCTOR CALLED.\n");
// }
//
// __attribute__((destructor)) static void layer_destructor() {
//     if (debug_file) {
//         fprintf(debug_file, "KROSSHAIR unloaded\n");
//         fclose(debug_file);
//     }
//     fprintf(stderr, "[KROSSHAIR] DESTRUCTOR CALLED.\n");
// }
